# Kubernetes Manifests (Reference Only)

⚠️ **Deprecated**

The Kubernetes manifests in this directory are kept for **learning and reference purposes only**.

The project has moved to using **Helm** as the supported and maintained deployment mechanism.



## Why this directory exists

- Demonstrates the transition from raw Kubernetes manifests to Helm charts
- Useful for understanding how Helm templates map to Kubernetes resources
- Preserved as part of the learning journey in this project
